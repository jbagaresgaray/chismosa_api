...
router.route('/contacts/deny/')
    .put(function(req, res) {
        ...
        if (errors) {
            ...
            if (connection) {
                var sSQL = 'DELETE FROM user_contacts WHERE (friend_one= \'' + req.body.user_id + '\' OR friend_two=\'' + req.body.user_id + '\') \
                            AND \
                            (friend_one=\'' + req.body.friend_id + '\' OR friend_two=\'' + req.body.friend_id + '\');';

                connection.query(sSQL, function(err, rows, result) {
                    if (err) throw err;
